...
function App() {
    ...
    const [container, setContainer] = useState("original");
    ...
    useEffect(() => {
        async function init() {
            ...
            try {
                ...
                if (cfgRes?.success && cfgRes.config?.container) {
                    setContainer(cfgRes.config.container);
                }
            } catch {
                // ignore
            }
        ...
        if (!binaryStatus.success) {
            return (
                <p className="text-red-600 text-sm">
                    Không tải được yt-dlp/ffmpeg: {binaryStatus.error}
                </p>
            );
        }
        return (
            <div className="text-xs text-gray-600 space-y-1">
                ...
                {binaryStatus.ffmpeg?.status === "ok" && (
                    <p>ffmpeg: {binaryStatus.ffmpeg.path}</p>
                )}
                {binaryStatus.ffmpeg?.status === "system" && (
                    <p>ffmpeg: dùng ffmpeg trong PATH hệ thống</p>
                )}
                {binaryStatus.ffmpeg?.status === "missing" && (
                    <p className="text-red-600">
                        Không tìm thấy ffmpeg. Hãy chạy script tải ffmpeg và
                        giải nén `ffmpeg.exe` vào thư mục `electron/bin` hoặc
                        cài ffmpeg vào PATH.
                    </p>
                )}
                {container !== "original" &&
                    binaryStatus.ffmpeg?.status === "missing" && (
                        <p className="text-yellow-600 text-sm">
                            Chọn định dạng xuất yêu cầu ffmpeg để chuyển đổi;
                            hiện chưa tìm thấy ffmpeg.
                        </p>
                    )}
            </div>
        );
            ...
            <header className="px-4 py-3 bg-slate-900 text-white flex items-center justify-between">
                <div>
                    ...
                    <p className="text-xs text-slate-300">
                        Tải YouTube: âm thanh, video, hoặc cả hai với chất lượng
                        tốt.
                    </p>
                </div>
            </header>
                        ...
                        <div>
                            ...
                            <select
                                ...
                                onChange={(e) => {
                                    ...
                                    setContainer(v);
                                    if (window.downloader?.updateConfig)
                                        window.downloader.updateConfig({
                                            container: v,
                                        });
                                }}
                            >
